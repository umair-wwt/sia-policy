...
TOPICS = {
    ...
    "status": ("Activate or suspend policies", "[defaults] policy_status controls new policies. Existing Active/Suspended status is preserved during ordinary updates. A corrective field update from a platform-managed state such as Error or Validating requests the configured policy_status; plan shows the change. To deliberately change selected policies, preview with sia plan --input input --update --set-policy-status Active, then use apply with the same flags. Suspended is also supported. Only managed or explicitly adopted policies can be updated. A created policy is not access-ready until its state is verified."),
    "inputs": ("Server lists", "servers.csv needs fqdn and either a principal column (an Identity role, or a group when [defaults] principal_type = \"group\") or a configured principal_template. domains.csv supplies shared domain accounts; strong_accounts.csv describes exceptions; groups.csv pins a directory for group principals only. CSV problems include the path and row. For a single machine use --server FQDN --principal NAME. Use --workgroup for a Windows target outside a domain. RDP is the default; SSH needs a username. To onboard only the strong accounts of a list of standalone servers, see sia help accounts."),
    "accounts": ("Strong accounts only (standalone servers)", "plan, apply and verify --accounts onboard only the strong accounts of the listed servers: the local administrator that already exists on each server is stored in SIA under the [defaults] strong_account_* naming convention (strong_account_type = \"credentials\": user name and password kept in SIA; \"vault\": a reference, after the optional Vault stage). No target set or policy is created, so --update, --drift, --adopt, --set-policy-status, --resume and --principal are rejected; --only vault or --only secrets limit the account stages.\n\nservers.csv needs only fqdn (add domain_joined = no for workgroup servers so the same file serves the later server run), or use --server FQDN --workgroup from a build job. Linux rows are skipped. Passwords come from the environment variable, the password file (name = the account name or the server FQDN) or a hidden prompt on apply. Re-running reports exists. The later normal plan/apply for the same servers creates the target set and policy and reports the account as exists. On each standalone server: the account must be in the local Administrators group, LocalAccountTokenFilterPolicy must be 1, and the SIA connector must reach WinRM (TCP 5985/5986)."),
    ...
    "connect": ("Connection troubleshooting", "connect-info exports gateway and connection details. --no-tenant works offline and leaves tenant status empty. A passing API check does not establish that a user can log in. Check role or group membership, policy status/access window, connector health, and connector-to-target WinRM or SSH reachability. Windows also needs the intended strong account and local-account remote administration prerequisites. Test one real session as a member of the allowed role or group."),
